...
export async function getHeatmapData(): Promise<Array<{ id: number; latitude: number; longitude: number }>> {
...
export async function getLogs(limit: number): Promise<Array<{ timestamp: string; message: string; level: string }>> {
