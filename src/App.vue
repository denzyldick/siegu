...
export default {
  components: { DeviceList, Map, Photos, People, Setting, Greet, Connect, FolderPicker, GuidedTour },
  data: () => ({
    logoUrl: logo,
    clean_install: false,
    scanning: false,
    ...
    <template v-if="clean_install">
          ...
          <v-col cols="12" sm="10" md="8" lg="6">
            <v-card variant="flat" rounded="xl" class="pa-8 border-subtle">
              <div class="text-center mb-8">
                ...
                <h2 class="text-h4 font-weight-bold text-zinc-primary">{{ $t('onboarding.add_media_title') }}</h2>
          ...
          <v-col cols="12" sm="10" md="8" lg="6">
            <v-card variant="flat" rounded="xl" class="pa-8 border-subtle">
              <div class="text-center mb-8">
                ...
                <h2 class="text-h4 font-weight-bold text-zinc-primary">{{ $t('onboarding.ai_title') }}</h2>
              ...
              <v-btn block color="black" height="56" class="siegu-btn mt-8" 
                ...
                {{ downloadedModels.length < 2 ? $t('onboarding.download_required') : $t('onboarding.continue') }}
              </v-btn>
            </v-card>
          </v-col>
        </v-row>
          ...
          <v-col cols="12" sm="10" md="8" lg="6">
            <v-card variant="flat" rounded="xl" class="pa-8 border-subtle">
              <div class="text-center mb-8">
                ...
                <h2 class="text-h4 font-weight-bold text-zinc-primary">{{ $t('onboarding.sync_title') }}</h2>
              ...
              <v-expand-transition>
                <div v-if="connectionMode === 'join'" class="mb-8">
                  <div class="text-caption font-weight-bold text-zinc-muted mb-4 tracking-widest uppercase text-center">{{ $t('onboarding.target_sync_folder') }}</div>
                  <v-card variant="flat" class="bg-zinc-50 border-subtle pa-4 rounded-xl d-flex align-center">
                    ...
                    <div class="flex-grow-1 overflow-hidden">
                      <div class="text-caption text-zinc-secondary">{{ $t('onboarding.sync_storage') }}</div>
                      <div class="text-body-2 font-weight-bold text-zinc-primary text-truncate">{{ syncPath || $t('onboarding.auto_select') }}</div>
                    </div>
              ...
              <v-fade-transition>
                <div v-if="deviceConnected" class="mb-6">
                  <div class="bg-success-light border-success pa-4 rounded-xl mb-4 text-center d-flex align-center justify-center">
                    ...
                    <span class="text-success font-weight-bold">{{ $t('onboarding.device_linked') }}</span>
                  </div>
      ...
      <v-container v-else-if="onboardingStep === 'finalize'" class="fill-height" fluid>
        <v-row justify="center">
          <v-col cols="12" sm="10" md="8" lg="6">
            <v-card variant="flat" rounded="xl" class="pa-8 border-subtle text-center">
              ...
              <h2 class="text-h3 font-weight-black text-zinc-primary mb-4">{{ $t('onboarding.ready_title') }}</h2>
          ...
          <v-col cols="auto">
            <v-menu offset-y transition="scale-transition">
              <template v-slot:activator="{ props }">
                <v-btn v-bind="props" color="#000000" theme="dark" variant="flat" :class="isMobile ? 'px-2' : 'px-4'" height="40" rounded="lg" data-tour="scan-button">
                ...
                <v-card-text class="pa-4">
                  <v-list density="compact" bg-color="transparent" class="pa-0">
                    <v-list-item class="px-0 mb-4">
                      ...
                      <v-list-item-title class="text-zinc-primary font-weight-bold">{{ $t('sync.file_scanner') }}</v-list-item-title>
                      <v-list-item-subtitle class="text-zinc-secondary">
                        {{ scanStatus === 'discovering' ? $t('sync.processing_folder', { current: scanProgress.current, total: scanProgress.total }) : scanStatus === 'indexing' ? $t('sync.switched_ai') : $t('sync.idle') }}
                      ...
                      <div v-if="scanStatus === 'discovering'" class="mt-2">
                        <div v-if="currentScanFile.total > 0" class="d-flex align-center justify-space-between mb-1">
                          <span class="text-caption text-zinc-muted text-truncate" style="max-width: 200px;">{{ currentScanFile.filename || $t('sync.scanning') }}</span>
                    ...
                    <v-list-item class="px-0 mb-4">
                      ...
                      <v-list-item-title class="text-zinc-primary font-weight-bold">{{ $t('sync.ai_intelligence') }}</v-list-item-title>
                      <v-list-item-subtitle class="text-zinc-secondary">
                        {{ indexingCount > 0 ? $t('sync.jobs_remaining', { count: formatIndexingCount(indexingCount) }) : (scanStatus === 'indexing' ? $t('sync.finalizing') : $t('sync.all_indexed')) }}
                      </v-list-item-subtitle>
                      ...
                      <div v-if="Object.keys(modelProgress).length > 0" class="mt-2">
                        <div v-for="(mp, key) in modelProgress" :key="key" class="d-flex align-center justify-space-between mb-1">
                          ...
                          <v-chip :color="mp.status === 'completed' ? 'success' : mp.status === 'error' ? 'error' : 'default'" size="x-small" variant="flat" class="text-white" density="compact">
                            {{ mp.status === 'completed' ? $t('settings.ready') : mp.status === 'running' ? `${mp.pending} ${$t('sync.jobs_left', { count: mp.pending })}` : mp.status }}
                          </v-chip>
                        </div>
                      </div>
                    </v-list-item>
                  ...
                  <div class="d-flex align-center justify-space-between mb-6">
                    <span class="text-caption text-zinc-muted">{{ $t('sync.last_sync', { time: lastScanTime }) }}</span>
                  </div>
            ...
            >
              ...
              <template v-slot:append-inner>
                <v-tooltip location="bottom" max-width="280">
                  <template v-slot:activator="{ props }">
                    <v-icon v-bind="props" size="18" color="#a1a1aa" class="cursor-pointer">mdi-help-circle-outline</v-icon>
                  </template>
                  <div class="pa-2">
              ...
              <template v-slot:prepend-item>
                ...
                <v-list-subheader v-if="!search" class="text-zinc-muted text-uppercase tracking-widest text-caption py-2">{{ $t('search.top_suggestions') }}</v-list-subheader>
              </template>
            </v-autocomplete>
            </div>
          </v-col>
              ...
              <v-menu :close-on-content-click="false" offset-y>
                ...
                <v-card min-width="250" border class="mt-2 border-subtle" color="surface" rounded="xl">
                  <v-list bg-color="transparent" density="compact" class="px-2 ga-2">
                    ...
                    <v-list-item class="px-0">
                      <v-switch v-model="filters.videosOnly" :label="$t('filters.videos_only')" color="#000000" hide-details density="compact" inset class="text-zinc-secondary px-2"></v-switch>
                    </v-list-item>
                    <v-divider class="border-subtle my-2"></v-divider>
                    <v-list-subheader class="text-zinc-muted text-uppercase tracking-widest text-caption px-0">{{ $t('filters.date_range') }}</v-list-subheader>
                    <v-list-item class="px-0">
                      <v-btn-toggle v-model="filters.dateRange" mandatory variant="flat" density="compact" class="ga-2 w-100 bg-transparent">
                        <v-btn value="all" size="x-small" class="siegu-btn flex-grow-1">{{ $t('filters.all') }}</v-btn>
                        <v-btn value="month" size="x-small" class="siegu-btn flex-grow-1">{{ $t('filters.month') }}</v-btn>
                        <v-btn value="year" size="x-small" class="siegu-btn flex-grow-1">{{ $t('filters.year') }}</v-btn>
                      </v-btn-toggle>
                    </v-list-item>
                    <v-divider class="border-subtle my-2"></v-divider>
                    <v-list-subheader class="text-zinc-muted text-uppercase tracking-widest text-caption px-0">{{ $t('filters.folder') }}</v-list-subheader>
                    <v-list-item class="px-0">
                      <v-select v-model="filters.folder" :items="directories" :placeholder="$t('filters.all_folders')" variant="solo-filled" density="compact" hide-details flat rounded="lg" class="siegu-field"></v-select>
                    </v-list-item>
                  </v-list>
        ...
        <v-main class="bg-siegu-main">
                  ...
                  <div class="d-flex align-center ga-2 min-width-0 flex-shrink-1" style="max-width: 55%;">
                    ...
                    <div class="text-caption font-weight-bold text-zinc-primary text-truncate">
                      ...
                      <template v-else-if="scanStatus === 'indexing' || indexingCount > 0">
                        ...
                        <span v-else class="text-zinc-muted font-weight-regular">{{ $t('sync.jobs_left', { count: formatIndexingCount(indexingCount) }) }}</span>
                      </template>
                    </div>
                  </div>
          ...
          <div data-tour="photos" class="w-100"><Photos v-if="current_page === 'home'" :search-query="search" :filters="filters" @clear-search="search = null" /></div>
      ...
      <div class="dock-container" v-if="!clean_install">
        <v-sheet class="dock d-flex justify-space-around align-center pa-2 rounded-pill mb-8" elevation="0" width="100%" max-width="380" color="surface">
          ...
          <v-btn icon variant="text" @click="current_page = 'people'" size="small" class="siegu-dock-btn" :class="{'siegu-dock-btn--active': current_page === 'people'}" data-tour="dock-people">
            <v-icon size="24">mdi-account-group-outline</v-icon>
          </v-btn>
          <v-btn icon variant="text" @click="current_page = 'location'" size="small" class="siegu-dock-btn" :class="{'siegu-dock-btn--active': current_page === 'location'}" data-tour="dock-map">
            <v-icon size="24">mdi-map-outline</v-icon>
          </v-btn>
          <v-btn icon variant="text" @click="current_page = 'devices'" size="small" class="siegu-dock-btn" :class="{'siegu-dock-btn--active': current_page === 'devices'}" data-tour="dock-devices">
            <v-icon size="24">mdi-laptop</v-icon>
          </v-btn>
          <v-btn icon variant="text" @click="current_page = 'settings'" size="small" class="siegu-dock-btn" :class="{'siegu-dock-btn--active': current_page === 'settings'}" data-tour="dock-settings">
    ...
    <v-dialog v-model="searchDisabledDialog.show" max-width="400" rounded="xl">
      <v-card color="surface" border class="border-subtle overflow-hidden">
        <v-card-item class="bg-zinc-100 py-4">
          ...
          <v-card-title class="text-h6 text-zinc-primary font-weight-bold">{{ $t('search.disabled_title') }}</v-card-title>
          <template v-slot:append>
            <v-btn icon="mdi-close" variant="text" size="small" @click="searchDisabledDialog.show = false"></v-btn>
          </template>
        </v-card-item>
        ...
        <v-card-actions class="pa-4 bg-zinc-50 border-top-subtle ga-2">
          <v-btn variant="flat" color="black" @click="searchDisabledDialog.show = false" class="siegu-btn flex-grow-1" height="44">{{ $t('search.disabled_got_it') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-app>
