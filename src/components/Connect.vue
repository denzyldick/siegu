  ...
  <div class="connect-wrapper">
    <v-dialog v-model="dialog" width="auto" scrim="black" transition="dialog-bottom-transition">
      ...
      >
        ...
        <div
          class="text-caption text-zinc-muted mb-2 uppercase tracking-widest"
          v-if="mode === 'host'"
        >
        ...
        <div
          ...
          v-if="passphrase.length > 0 && mode === 'host'"
        >
          <v-chip
        ...
        <div
          class="d-flex justify-center mb-6 flex-column ga-4"
        ...
        <div class="text-caption text-zinc-muted mb-1 text-center py-2" v-if="connectionStatus">
          <v-progress-circular
            v-if="!isConnected && connectionStatus !== $t('connect.disconnected')"
            indeterminate
            color="black"
            size="16"
            width="2"
        ...
        <v-btn
          v-if="isConnected || (connectionStatus && connectionStatus !== 'Disconnected')"
          variant="flat"
          color="black"
          size="small"
    ...
    <div v-if="embedded" class="w-100">
      ...
      <div
        class="text-caption text-zinc-muted mb-2 uppercase tracking-widest text-center"
        v-if="mode === 'host'"
      >
        {{ $t('connect.manual_phrase') }}
      </div>
      ...
      <div class="d-flex justify-center mb-6 flex-column ga-4" v-if="mode === 'join'">
        ...
        <v-btn
          ...
          block
          :loading="loading"
          :disabled="!joinPassphrase || isConnected"
        >
          <div class="d-flex align-center">
            <div class="siegu-icon-circle mr-3">
              <v-icon size="14">mdi-link-variant</v-icon>
    ...
    async dialog(val) {
      if (val) {
        this.unlisten = await listen('webrtc-state', (event) => {
          ...
          if (
            event.payload.toLowerCase().includes('error') ||
        ...
        this.unlistenSync = await listen('sync-progress', (event) => {
          ...
          if (
            event.payload.status === 'Up to date' ||
            event.payload.status.startsWith('Finished')
          ) {
            this.syncing = false;
            setTimeout(() => {
