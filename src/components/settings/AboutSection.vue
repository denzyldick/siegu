    ...
    <v-card-item class="py-4">
      <template v-slot:prepend>
        <v-avatar color="surface" size="32" class="mr-3">
          <v-icon color="on-surface" size="small">mdi-information-outline</v-icon>
        </v-avatar>
      </template>
      ...
      <v-expansion-panels flat class="bg-transparent">
        <v-expansion-panel class="bg-transparent">
          ...
          <v-expansion-panel-text class="text-medium-emphasis">
            ...
            >
              <span class="text-body-2 text-high-emphasis">{{ dep.name }}</span>
              <span class="text-caption text-medium-emphasis">{{ dep.license }}</span>
            </div>
