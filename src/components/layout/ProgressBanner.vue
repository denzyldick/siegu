...
<template>
  <v-slide-y-reverse-transition>
    <div v-if="scanStore.isActive" class="progress-banner" data-tour="scan-progress">
      <div class="progress-banner-inner px-4 py-2">
        <div class="d-flex align-center justify-space-between flex-wrap ga-2">
          <div class="d-flex align-center ga-2 min-width-0 flex-shrink-1" style="max-width: 55%">
            <v-progress-circular indeterminate size="16" width="2" color="black" />
            <div class="text-caption font-weight-bold text-zinc-primary text-truncate">
              <template v-if="scanStore.status === 'scanning'">
              ...
              <template v-else-if="scanStore.status === 'indexing' || scanStore.indexingCount > 0">
                ...
                <span class="text-zinc-muted font-weight-regular">
                  {{ t('sync.jobs_left', { count: normalizeIndexingCount(scanStore.indexingCount).toLocaleString() }) }}
                </span>
              </template>
            </div>
          </div>
          <div class="d-flex align-center ga-3 flex-shrink-0">
            <span v-if="displayEta" class="text-caption text-zinc-muted">
              ~{{ displayEta }}
            </span>
            <span v-else-if="scanStore.indexingCount > 0" class="text-caption text-zinc-muted">
              {{ t('sync.jobs_left', { count: normalizeIndexingCount(scanStore.indexingCount).toLocaleString() }) }}
            </span>
          </div>
        </div>
