...
open class BuildTask : DefaultTask() {
    ...
    fun assemble() {
        ...
        val skipRust = project.hasProperty("skipRustBuild") &&
            project.property("skipRustBuild").toString().toBoolean()

        if (skipRust) {
            logger.lifecycle("Skipping Rust build for $target ($abi) (skipRustBuild=true)")
        } else {
        ...
        if (soFile.exists()) {
            ...
            logger.warn("No .so found at ${soFile.absolutePath} — jniLibs not updated")
        } else {
