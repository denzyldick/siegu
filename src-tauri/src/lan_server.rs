...
/// Starts a local WebSocket signaling server on the given port (0 = random).
/// Returns the actual port the server is listening on.
/// Accepts connections at `ws://0.0.0.0:{port}/{room_id}`.
...
mod tests {
    ...
    async fn test_peer_disconnected_notification() {
        ...
        // Use unsplit stream for B so we can send join properly
        ...
        // Client A should get PeerDisconnected
        let received = recv_signal(&mut ws_a).await;
        match received {
